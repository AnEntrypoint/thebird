...
(async () => {
  ...
  console.log('=== pass 7: Firefox + polyfills (UA/net/tls/dgram/v8/heap/cluster/inspector) ===');
  const ff7 = await import('file:///' + path.resolve('docs/shell-node-firefox.js').replace(/\\/g, '/'));
  const n7 = await import('file:///' + path.resolve('docs/shell-node-net.js').replace(/\\/g, '/'));
  const prof7 = await import('file:///' + path.resolve('docs/shell-node-profiler.js').replace(/\\/g, '/'));
  const clu7 = await import('file:///' + path.resolve('docs/shell-node-cluster.js').replace(/\\/g, '/'));
  const ins7 = await import('file:///' + path.resolve('docs/shell-node-inspector.js').replace(/\\/g, '/'));
