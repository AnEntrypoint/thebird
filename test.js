...
(async () => {
  ...
  const d11=dns11.makeDns(); const nl=nat11.makeNativeLoader();
  [[pfsX.read('/proc/self/cmdline').includes('node'),'cmdline'],[pfsX.read('/etc/hosts').includes('localhost'),'hosts'],[pfsX.read('/etc/passwd').includes('root'),'passwd'],[nl.list().length>0,'native'],[l1[0].includes('Linux'),'uname'],[/^\d+$/.test(l2[0].trim()),'nproc'],[l3.some(x=>x.includes('Mem:')),'free'],[typeof d11.promises.lookup==='function','dns']].forEach(([v,n])=>assert(v,n));
