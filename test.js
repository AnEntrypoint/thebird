...
(async () => {
  ...
  assert(ran && runner.results.pass===1 && runner.results.fail===1, 'runner');
  const m10=runner.mock.fn(x=>x*2); m10(5); assert(m10.mock.calls[0].arguments[0]===5, 'mock');
  const mt=new ue10.MIMEType('text/html; charset=utf-8');
  [[typeof tr10.makeTapReporter(null).ok==='function','tap'],[ue10.styleText('red','hi').includes('\x1b[31m'),'style'],[ue10.stripVTControlCharacters('\x1b[31mhi\x1b[0m')==='hi','stripVT'],[mt.type==='text'&&mt.params.get('charset')==='utf-8','MIME'],[Array.isArray(ue10.getCallSites(3)),'callsites']].forEach(([v,n])=>assert(v,n));
  const logged=[]; const ce=ue10.makeConsoleExtras({log:s=>logged.push(s)},{write:s=>logged.push(s)}); ce.time('x'); ce.timeEnd('x'); ce.count('i');
  assert(logged.some(l=>l.startsWith('x:'))&&logged.some(l=>l.includes('i:')), 'console extras');
