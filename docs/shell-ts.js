...
export async function stripTypes(src,opts={}){
  try{const s=await getStripper();if(s?.transform){const r=s.transform(src,{transforms:['typescript',...(opts.jsx?['jsx']:[])],jsxRuntime:'automatic'});return r.code;}}catch{/* swallow: sucrase transform failed, fall through to the regex-based stripper below (warning logged there) */}
  console.warn('shell-ts: sucrase unavailable or failed, falling back to regex TS stripper - complex syntax may be mis-transpiled');
