...
export function makeReadline(term,proc){
  return {
    createInterface:({input,output,prompt:_prompt='> ',terminal:useTerm}={})=>{
      ...
      const rl={
        ...
        prompt:()=>(output||term)?.write?.(_prompt),
...
export function makeV8Mod(){
  return {serialize:o=>new TextEncoder().encode(JSON.stringify(o)),deserialize:b=>JSON.parse(new TextDecoder().decode(b))};
}
...
export function extendProcessExtras(proc,ctx){
  ...
  // resourceUsage: all fields are synthetic stubs (browser sandbox cannot measure CPU time, page faults, or I/O counters). Do not use for profiling or tuning.
  proc.resourceUsage=()=>({userCPUTime:0,systemCPUTime:0,maxRSS:50000,sharedMemorySize:0,unsharedDataSize:0,unsharedStackSize:0,minorPageFault:0,majorPageFault:0,swappedOut:0,fsRead:0,fsWrite:0,ipcSent:0,ipcReceived:0,signalsCount:0,voluntaryContextSwitches:0,involuntaryContextSwitches:0,_synthetic:true});
  ...
  const origOff=proc.off?.bind(proc);
  ...
  // .on() diverted handlers into sigH instead of the base EventEmitter's own
  // listener list, so the base .off()/removeListener() (which only knows
  // about that list) could never actually remove them -- a registered
  // process.on('exit', fn) was permanently un-removable. Mirror the same
  // sigH-aware removal here.
  proc.off=(ev,fn)=>{if(sigH[ev])sigH[ev]=sigH[ev].filter(h=>h!==fn);origOff?.(ev,fn);return proc;};
  proc.removeListener=proc.off;
  proc.removeAllListeners=ev=>{if(ev)delete sigH[ev];else for(const k of Object.keys(sigH))delete sigH[k];return proc;};
