...
export function makePnpmLayout(fs,snap,persist,ctx){
  ...
  return{
    ...
    resolve(name,ver){
      const base=ctx.cwd.replace(/^\//,'').replace(/\/$/,'');
      if(ver)return'/'+storeBase()+'/'+name+'@'+ver+'/node_modules/'+name;
      return'/'+(base?base+'/':'')+'node_modules/'+name;
    }
  };
}
...
export function parseWorkspaces(snap,cwd=''){
  ...
  let patterns=[];
  if(pj){try{const p=JSON.parse(pj);if(Array.isArray(p.workspaces))patterns=p.workspaces;else if(p.workspaces?.packages)patterns=p.workspaces.packages;}catch{/* swallow: package.json present but not valid JSON, fall through to pnpm-workspace.yaml (patterns stays []) */}}
