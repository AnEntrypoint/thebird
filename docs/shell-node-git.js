...
async function loadGit(){
  if(!gitPromise)gitPromise=Promise.all([
    ...
    import('./vendor/esm/isomorphic-git-http-web.mjs').then(m=>m.default||m).catch(e=>{console.warn('[git] http transport unavailable; https clone will fail',e);return null;}),
...
export function makeGit(fs){
  const fsAdapter={
    promises:{
      async readFile(path,opts){try{const enc=typeof opts==='string'?opts:opts?.encoding;const d=fs.readFileSync(path);if(enc==='utf8'||enc==='utf-8')return typeof d==='string'?d:new TextDecoder().decode(d);return typeof d==='string'?new TextEncoder().encode(d):d;}catch(e){if(e.code)throw e;throw new Error('read failed: '+e.message);}},
  ...
  return{
    ...
    branch:wrap('branch'),
    deleteBranch:wrap('deleteBranch'),
    hashBlob:wrap('hashBlob'),
    ...
    diff:async opts=>{const rows=await wrap('statusMatrix')(opts);return rows.filter(([,head,work])=>head===1&&work!==1).map(([path])=>path);},
    preload:preloadGit,
  };
}
