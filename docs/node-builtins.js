...
const toKey = p => { const k = String(p).replace(/^\//, ''); if (k === '') throw new Error('Invalid path: empty key'); return k; };
...
export function createFs() {
  ...
  return {
    readFileSync: (p, enc) => {
      ...
      return enc ? data : new TextEncoder().encode(data);
    },
...
export function createUrl() {
  return {
    ...
    format: o => {
      ...
      for (const [k, v] of Object.entries(o)) { try { u[k] = v; } catch { /* swallow: URL setter rejected an invalid/read-only field, skip it and keep formatting the rest */ } }
      return u.href;
    },
...
export function createBuffer() {
  ...
  Buf.from = (d, enc) => {
    ...
    if (typeof d !== 'string') throw new TypeError('Buffer.from() argument must be a string, Array, or Uint8Array');
    if (enc === 'base64') return new Buf(Uint8Array.from(atob(d), c => c.charCodeAt(0)));
    if (enc === 'hex') { const m = d.match(/.{2}/g); if (!m) return new Buf(0); return new Buf(m.map(h => parseInt(h, 16))); }
