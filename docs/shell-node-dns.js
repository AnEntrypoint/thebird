...
async function query(name,type='A'){
  ...
  try{
    const r=await fetch(`${DOH}?name=${encodeURIComponent(name)}&type=${t}`,{headers:{accept:'application/dns-json'},signal:AbortSignal.timeout(5000)});
    const j=await r.json();
    return j.Answer||[];
  }catch(e1){
    console.warn('DNS DOH primary provider failed: '+(e1&&e1.message)+', retrying fallback');
    try{
      const r=await fetch(`${DOH_FALLBACK}?name=${encodeURIComponent(name)}&type=${t}`,{signal:AbortSignal.timeout(5000)});
      const j=await r.json();
      return j.Answer||[];
    }catch(e2){
      throw Object.assign(new Error('query'+type+' '+name+' EAI_AGAIN'),{code:'EAI_AGAIN',hostname:name,syscall:'query'+type});
    }
  }
}
...
export function makeDns(){
  ...
  return{
    ...
    get lookup(){const p=this.promises;return(h,o,cb)=>{if(typeof o==='function'){cb=o;o={};}p.lookup(h,o).then(r=>cb(null,r.address,r.family),cb);};},
