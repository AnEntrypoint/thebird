...
const PM_VERSIONS={bun:'1.3.8',pnpm:'9.15.0',yarn:'1.22.22',npm:'10.9.2'};
...
export function detectPm(snap,cwd=''){
  ...
  if(pj){try{const p=JSON.parse(pj);if(p.packageManager){const m=p.packageManager.match(/^(\w+)@([\d.]+)/);if(m)return{pm:m[1],version:m[2],source:'packageManager'};}}catch{/* swallow: package.json present but not valid JSON, fall through to lockfile detection below */}}
...
export function makePmDispatcher(term,fs,persist,ctx){
  ...
  // intentionally simplified, non-pm-native lockfile schema (not real bun.lock/pnpm-lock.yaml/yarn.lock shape) -- nothing in thebird parses it back
  ...
  const cmds={
    ...
    async install(pm,args){const pj=readPj();const deps={...(pj.dependencies||{}),...(pj.devDependencies||{})};term.write(`${pm} install — ${Object.keys(deps).length} packages staged (resolved at import time via esm.sh)\r\n`);writeLock(pm,pj.dependencies||{});return 0;},
    async run(pm,args){const script=args[0];const pj=readPj();const s=pj.scripts?.[script];if(!s){term.write(pm==='bun'?`error: Script not found "${script}"\r\n`:`${pm} run: no script '${script}'\r\n`);return 1;}term.write(`$ ${s}\r\n`);const prevEnv={...ctx.env};ctx.env['npm_lifecycle_event']=script;ctx.env['npm_lifecycle_script']=s;ctx.env['npm_package_name']=pj.name||'';ctx.env['npm_package_version']=pj.version||'';ctx.env['npm_config_user_agent']=`${pm}/${PM_VERSIONS[pm]||'1.0.0'} npm/? node/${NODE_VERSION} win32 x64`;try{return ctx.exec?.(s)||0;}finally{ctx.env=prevEnv;}},
