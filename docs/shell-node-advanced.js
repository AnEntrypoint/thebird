...
export function makeVmModule(){
  ...
  const registry=typeof FinalizationRegistry!=='undefined'?new FinalizationRegistry(iframe=>{try{iframe.remove();}catch{/* swallow: GC-triggered cleanup, iframe may already be detached/removed by an earlier explicit teardown */}}):{register(){}};
  ...
  const mkIframe=ctx=>{if(!hasDom)return{iframe:null,win:globalThis};const f=document.createElement('iframe');f.style.display='none';f.setAttribute('sandbox','allow-scripts');document.body.appendChild(f);const win=f.contentWindow;if(ctx)for(const k of Object.keys(ctx))win[k]=cloneAcross(ctx[k]);registry.register(ctx||{},f);return{iframe:f,win};};
  ...
  return{
    ...
    runInNewContext:(code,ctx={})=>{const{iframe,win}=mkIframe(ctx);if(!iframe)throw new Error('vm.runInNewContext: DOM context required for context isolation');const r=win.eval(code);syncBack(ctx,win);return cloneAcross(r);},
