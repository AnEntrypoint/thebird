...
function inspectPrimitive(v, colors) {
  ...
  const t = typeof v;
  if (t === 'string') return paint(quoteString(v), COLORS.string, colors);
  ...
  if (t === 'function') {
    if (/^class\s/.test(Function.prototype.toString.call(v))) {
      const parent = Object.getPrototypeOf(v);
      const ext = typeof parent === 'function' && parent !== Function.prototype && parent.name ? ' extends ' + parent.name : '';
      return paint('[class ' + (v.name || '(anonymous)') + ext + ']', COLORS.special, colors);
    }
    return paint('[Function' + (v.name ? ': ' + v.name : ' (anonymous)') + ']', COLORS.special, colors);
  }
  return null;
}
...
export function inspect(v, opts = {}, depth = 0, seen = new Map(), refCount = { n: 0 }) {
  ...
  if (Array.isArray(v)) {
    ...
    if (v.length <= 6 && oneLine.length <= breakLength) return oneLine;
    const indent = '  '.repeat(depth + 1);
    const canGroup = items.every(s => !s.includes('\n'));
    if (canGroup) {
      const maxLen = Math.max(...items.map(s => s.length));
      const actualMax = maxLen + 2;
      const approxCharHeights = 2.5;
      const biasedMax = Math.sqrt(approxCharHeights * 2.5 * actualMax) / actualMax;
      const columns = Math.max(1, Math.min(
        Math.round(Math.sqrt(biasedMax * items.length) / biasedMax),
        Math.floor((breakLength - indent.length) / actualMax),
        16 - depth,
        items.length
      ));
      if (columns > 1) {
        ...
        for (let i = 0; i < items.length; i += columns) {
          ...
          rows.push(indent + row.map((s, j) => i + j < items.length - 1 ? (s + ',').padEnd(actualMax) : s).join('').trimEnd());
        }
        return '[\n' + rows.join('\n') + '\n' + '  '.repeat(depth) + ']';
      }
    }
...
export function format(...args) {
  ...
  let i = 1;
  const out = fmt.replace(/%[csdifjoO%]/g, m => {
    if (m === '%%') return '%';
...
export async function getFflate() {
...
export function createZlib(Buf) {
  ...
  return {
    ...
    createGzip: () => { throw new Error('zlib.createGzip() streaming not supported in browser - use gzipSync() for buffered compression or async gzip(buf, cb)'); },
    createGunzip: () => { throw new Error('zlib.createGunzip() streaming not supported in browser - use gunzipSync() for buffered decompression or async gunzip(buf, cb)'); },
