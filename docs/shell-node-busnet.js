...
let _origin=null;
function getOrigin(){if(_origin)return _origin;if(typeof globalThis.crypto?.randomUUID!=='function')throw new Error('busnet: crypto.randomUUID unavailable (insecure context); cannot mint a collision-safe origin id');_origin=globalThis.crypto.randomUUID().slice(0,8);return _origin;}
...
function handleMsg(m){
  ...
  if(m.type==='data'&&connections.has(m.id)&&m.from===connections.get(m.id).remote&&m.to===getOrigin()){connections.get(m.id)?._onData(m.data);return;}
  if(m.type==='data-local'&&connections.has(m.id)){connections.get(m.id)?._onData(m.data);return;}
  if(m.type==='close'&&connections.has(m.id)){const c=connections.get(m.id);c._dead=true;c._onClose();connections.delete(m.id);return;}
...
const BUSNET_HIGH_WATER=16*1024*1024;

function createConnection(id,remote,fromServer=false){
  const handlers={data:[],end:[],close:[],open:[],error:[],drain:[]};
  const conn={
    id,remote,fromServer,readable:true,writable:true,
    _queuedBytes:0,_dead:false,
    ...
    write(data){
      if(conn._dead||(conn._peerId&&!connections.has(conn._peerId))){queueMicrotask(()=>conn._onError(new Error('busnet: peer disconnected')));return false;}
      const len=(data&&data.length)||0;
      const peer=connections.get(conn._peerId);
      if(peer){
        conn._queuedBytes+=len;
        queueMicrotask(()=>{peer._onData(data);conn._queuedBytes-=len;if(conn._queuedBytes<0)conn._queuedBytes=0;if(conn._queuedBytes<=BUSNET_HIGH_WATER)for(const h of handlers.drain)h();});
        return conn._queuedBytes<=BUSNET_HIGH_WATER;
      }
      // Cross-tab BroadcastChannel path: fire-and-forget, no delivery ack API exists,
      // so this stays best-effort (matches shell-node-net.js's documented WS-send caveat).
      post({type:'data',id,data,from:getOrigin(),to:remote});
      return true;
    },
    end(data){if(data)conn.write(data);conn._dead=true;for(const h of handlers.close)h();connections.delete(id);},
    destroy(){conn._dead=true;for(const h of handlers.close)h();connections.delete(id);},
    _onOpen(){for(const h of handlers.open)h();},
    _onData(d){for(const h of handlers.data)h(d);},
    _onClose(){conn._dead=true;for(const h of handlers.close)h();},
