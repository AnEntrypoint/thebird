...
export function makeForkIpc(proc){
  ...
  proc.send=msg=>{if(!proc.connected)return false;bc.postMessage({from:isChild?'child':'parent',to:isChild?'parent':'child',msg});return true;};
  proc.on=(ev,fn)=>{if(ev==='message'||ev==='disconnect')(handlers[ev]=handlers[ev]||[]).push(fn);return proc;};
  proc.disconnect=()=>{bc.postMessage({from:isChild?'child':'parent',to:isChild?'parent':'child',type:'disconnect'});proc.connected=false;for(const h of handlers.disconnect)h();};
  proc.connected=true;
