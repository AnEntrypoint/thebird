...
class Readable{
  ...
  pipe(dest){if(!dest||!dest.write||!dest.on)throw new Error('pipe destination must implement write() and on()');if(this._pipeDest)throw new Error('pipe: only one destination supported; call unpipe() first');this._pipeDest=dest;this.on('data',c=>{if(dest.write(c)===false)this.pause();});this.on('end',()=>dest.end?.());dest.on('drain',()=>this.resume());return dest;}
  unpipe(dest){if(!dest||this._pipeDest===dest)this._pipeDest=null;return this;}
...
class Writable{
  ...
  write(chunk,enc,cb){if(typeof enc==='function'){cb=enc;enc=null;}if(this.writableEnded){const e=Object.assign(new Error('write after end'),{code:'ERR_STREAM_WRITE_AFTER_END'});queueMicrotask(()=>cb?.(e));return false;}this._pending++;
    // Capture the queue depth at enqueue time. _write may invoke its callback
    // SYNCHRONOUSLY (the default writer does), which decrements _pending before we
    // return — so returning `this._pending<HWM` after the call always read ~0 and
    // never signalled backpressure. Decide backpressure on the depth as queued.
    const queuedDepth=this._pending;
    this._write(chunk,enc,err=>{this._pending--;cb?.(err);if(this._pending===0)this.emit('drain');});
    return queuedDepth<this.writableHighWaterMark;}
...
export function extendFsStreams(fs,Buf){
  fs.createReadStream=(path,opts={})=>{const r=new Readable();if(opts.encoding)r.setEncoding(opts.encoding);queueMicrotask(()=>{try{const data=fs.readFileSync(path);const buf=typeof data==='string'?Buf.from(data):data;const start=opts.start||0;const end=opts.end??buf.length;const fd=_nextFd++;r.emit('open',fd);r.fd=fd;r.push(buf.slice(start,end));r.push(null);}catch(e){if(e&&e.code==='ENOENT'){e.message=`ENOENT: no such file or directory, open '${path}'`;e.errno=-2;e.syscall='open';e.path=path;}r.destroy(e);}});return r;};
  // Unlike real Node (which streams each chunk directly to the fd), IndexedDB has
  // no incremental-append primitive: every chunk is buffered in memory here and the
  // whole file is written once via writeFileSync at 'finish'. Fine for typical shell
  // usage; a very large stream will hold its entire content in memory before flush.
