let smMod=null;let smPromise=null;const consumers=new Map();const lastAccess=new Map();const CAP=5;const CONSUMER_CACHE_MAX=50;
...
async function consumerFor(url,snapFn){
  if(consumers.has(url)){lastAccess.set(url,Date.now());return consumers.get(url);}
  if(consumers.size>CONSUMER_CACHE_MAX){let lruKey=null,lruTime=Infinity;for(const k of consumers.keys()){const t=lastAccess.get(k)||0;if(t<lruTime){lruTime=t;lruKey=k;}}consumers.delete(lruKey);lastAccess.delete(lruKey);console.warn('[srcmap] consumer cache full ('+CONSUMER_CACHE_MAX+' entries); evicting least-recently-used entry: '+lruKey);}
  if(!smMod)smMod=await loadSm();
  let src='';
  try{
  ...
  if(!src)return null;
  try{const raw=JSON.parse(src);const c=new smMod.SourceMapConsumer(raw);consumers.set(url,c);lastAccess.set(url,Date.now());return c;}catch{return null;}
}
...
export function installSourceMapStacks(snapFn){
  ...
  Error.prepareStackTrace=(err,frames)=>{
    ...
    const mapped=frames.map(f=>{
      ...
      depth++;
      const c=consumers.get(mapUrl);if(!c)return f;lastAccess.set(mapUrl,Date.now());
  ...
  Promise.resolve().then(async()=>{if(!smMod)smMod=await loadSm();const snap=snapFn();for(const [key,src] of Object.entries(snap)){const mapUrl=extractMapUrl(src);if(mapUrl)await consumerFor(mapUrl,snapFn);}});
}

export function clearSourceMapCache(){consumers.clear();lastAccess.clear();}
