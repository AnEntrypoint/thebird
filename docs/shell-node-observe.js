export function makeDebugRegistry(ctx){
  ...
  // Instance-scoped home: ctx (createNodeEnv's own {ctx,term} param, the same
  // per-shell-instance context object createShell builds) when supplied.
  // window.__debug.node is a last-instance-wins devtools-console convenience
  // for live inspection (nothing in this codebase reads it back
  // programmatically) -- kept as a secondary mirror, not the source of truth,
  // so a multi-terminal page's registries no longer clobber each other there.
  if(ctx) ctx.debugReg=reg;
  if(typeof window!=='undefined'){if (!window.__debug) window.__debug = {};window.__debug.node=reg;}
  return reg;
}

export function makeDiagnosticsChannel(){
  const channels=new Map();
  const ch=name=>{if(!channels.has(name))channels.set(name,{name,subs:new Set(),subscribe(fn){this.subs.add(fn);return this;},unsubscribe(fn){this.subs.delete(fn);return this;},publish(msg){for(const fn of this.subs)try{fn(msg,name);}catch{/* swallow: a misbehaving subscriber must not block delivery to other diagnostics_channel listeners */}},hasSubscribers(){return this.subs.size>0;}});return channels.get(name);};
  return{
    channel:ch,
...
/* fs.watch polyfill: IndexedDB-snapshot polling at a fixed 500ms interval, not real inotify — events lag up to 500ms behind the actual write, and a rapid write+revert within one poll window is coalesced away (no event fires), unlike real inotify which reports both. This is an honest degraded implementation, not a bug: no OS-level FS notification API is reachable from browser JS. */
export function makeFsWatchReal(getSnap){
  ...
  return (path,opts={},listener)=>{if(typeof opts==='function'){listener=opts;opts={};}const normalized=path.replace(/^\/+/,'').replace(/\/$/,'');const handlers={change:listener?[listener]:[],error:[],close:[]};const w={path:normalized,recursive:!!opts.recursive,handlers,on:(ev,fn)=>{(handlers[ev]=handlers[ev]||[]).push(fn);return w;},close:()=>{const i=watchers.indexOf(w);if(i>=0)watchers.splice(i,1);if(watchers.length===0&&intervalId!==null){clearInterval(intervalId);intervalId=null;lastSnap=null;}for(const h of handlers.close)h();},ref:()=>w,unref:()=>w};watchers.push(w);if(watchers.length===1&&intervalId===null){intervalId=setInterval(tick,500);}return w;};
}
