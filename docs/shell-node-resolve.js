...
export function resolvePackageEntry(snap, pkgDir) {
  ...
  if (pj.exports) {
    ...
    const err = new Error('No "exports" main defined in ' + pjKey);
    err.code = 'ERR_PACKAGE_PATH_NOT_EXPORTED';
    throw err;
  }
  const base = pkgDir + '/' + (pj.main || 'index.js').replace(/^\.\//, '');
  const baseKey = toKey(base);
  const candidates = [baseKey, baseKey + '.js', baseKey + '.json', baseKey + '.mjs', baseKey + '.cjs', baseKey + '/index.js', baseKey + '/index.json', baseKey + '/index.mjs'];
  for (const c of candidates) if (c in snap) return '/' + c;
  return base;
}
