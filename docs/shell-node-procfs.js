// makeProcFs returns a synthetic /proc and /etc virtual filesystem backed by
// browser APIs (performance.memory, navigator.hardwareConcurrency, etc.).
// All paths are synthetic; distinctive markers: vendor_id=BrowserCPU,
// idbfs mount, hostname=thebird. Machine-readable detection: after wireProcFs()
// the patched fs object carries fs.isProcSynthetic === true.
export function makeProcFs(proc){
  ...
  const gen={
    ...
    'proc/meminfo':()=>{const m=performance.memory||{totalJSHeapSize:0,jsHeapSizeLimit:0,usedJSHeapSize:0};return`MemTotal:       ${(m.jsHeapSizeLimit/1024)|0} kB\nMemFree:        ${((m.jsHeapSizeLimit-m.usedJSHeapSize)/1024)|0} kB\nMemAvailable:   ${((m.jsHeapSizeLimit-m.usedJSHeapSize)/1024)|0} kB\nBuffers:               0 kB\nCached:                0 kB\n`;},
