...
function sha256Core(msg, IV, outLen) {
  const p = padMsg(msg, 64, 8); const v = new DataView(p.buffer);
  ...
  return outLen === 32 ? out : out.slice(0, outLen);
}

export function sha256(msg) {
  return sha256Core(msg, [0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19], 32);
}

export function sha224(msg) {
  return sha256Core(msg, [0xc1059ed8, 0x367cd507, 0x3070dd17, 0xf70e5939, 0xffc00b31, 0x68581511, 0x64f98fa7, 0xbefa4fa4], 28);
}
...
function sha512Core(msg, IV, outLen) {
  ...
  return outLen === 64 ? out : out.slice(0, outLen);
}

export function sha512(msg) {
  return sha512Core(msg, [0x6a09e667f3bcc908n, 0xbb67ae8584caa73bn, 0x3c6ef372fe94f82bn, 0xa54ff53a5f1d36f1n, 0x510e527fade682d1n, 0x9b05688c2b3e6c1fn, 0x1f83d9abfb41bd6bn, 0x5be0cd19137e2179n], 64);
}

export function sha384(msg) {
  return sha512Core(msg, [0xcbbb9d5dc1059ed8n, 0x629a292a367cd507n, 0x9159015a3070dd17n, 0x152fecd8f70e5939n, 0x67332667ffc00b31n, 0x8eb44a8768581511n, 0xdb0c2e0d64f98fa7n, 0x47b5481dbefa4fa4n], 48);
}
...
function bytesToBase64(arr) { const chunk = 8192; let s = ''; for (let i = 0; i < arr.length; i += chunk) s += btoa(String.fromCharCode(...arr.slice(i, i + chunk))); return s; }

const HASH_IMPLS = { sha1: { fn: sha1, size: 64, len: 20 }, sha256: { fn: sha256, size: 64, len: 32 }, sha224: { fn: sha224, size: 64, len: 28 }, sha512: { fn: sha512, size: 128, len: 64 }, sha384: { fn: sha384, size: 128, len: 48 }, md5: { fn: md5, size: 64, len: 16 } };
...
export function createHash(alg) {
  ...
  return {
    ...
    digest(enc) { const total = chunks.reduce((s, c) => s + c.length, 0); const buf = new Uint8Array(total); let off = 0; for (const c of chunks) { buf.set(c, off); off += c.length; } const out = spec.fn(buf); if (enc === 'hex') return [...out].map(b => b.toString(16).padStart(2, '0')).join(''); if (enc === 'base64') return bytesToBase64(out); if (enc === 'base64url') return bytesToBase64(out).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, ''); return out; },
  };
}
...
export function createHmac(alg, key) {
  ...
  return {
    update(data) { chunks.push(toBytes(data)); return this; },
    digest(enc) { const total = chunks.reduce((s, c) => s + c.length, 0); const buf = new Uint8Array(total); let off = 0; for (const c of chunks) { buf.set(c, off); off += c.length; } const inner = spec.fn(concat(ipad, buf)); const out = spec.fn(concat(opad, inner)); if (enc === 'hex') return [...out].map(b => b.toString(16).padStart(2, '0')).join(''); if (enc === 'base64') return bytesToBase64(out); if (enc === 'base64url') return bytesToBase64(out).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, ''); return out; },
  };
}
...
export function pbkdf2Sync(password, salt, iterations, keylen, digest) {
  ...
  // Pre-compute HMAC key padding once; reuse ipad/opad across all iterations
  let k = toBytes(password);
  if (k.length > spec.size) k = spec.fn(k);
  if (k.length < spec.size) { const pad = new Uint8Array(spec.size); pad.set(k); k = pad; }
  const ipad = new Uint8Array(spec.size), opad = new Uint8Array(spec.size);
  for (let i = 0; i < spec.size; i++) { ipad[i] = k[i] ^ 0x36; opad[i] = k[i] ^ 0x5c; }
  const hmac = data => spec.fn(concat(opad, spec.fn(concat(ipad, data))));
  for (let i = 1; i <= blocks; i++) {
    const block = new Uint8Array(saltB.length + 4); block.set(saltB); const dv = new DataView(block.buffer); dv.setUint32(saltB.length, i, false);
    let U = hmac(block);
    let T = new Uint8Array(U);
    for (let j = 1; j < iterations; j++) { U = hmac(U); for (let k = 0; k < hLen; k++) T[k] ^= U[k]; }
    out.set(T, (i - 1) * hLen);
  }
...
export function randomBytes(n, cb) {
  // Honest interface: randomBytes promises cryptographic quality. If the platform
  // CSPRNG (crypto.getRandomValues) is unavailable, FAIL LOUDLY rather than silently
  // degrade to Math.random() — a caller doing key/nonce/token generation must never
  // unknowingly get predictable bytes.
  const c = globalThis.crypto;
  const makeBuf = out => (globalThis.Buffer && typeof globalThis.Buffer.from === 'function') ? globalThis.Buffer.from(out) : out;
  if (!c || typeof c.getRandomValues !== 'function') {
    const err = new Error('crypto.randomBytes: no cryptographically secure RNG available (crypto.getRandomValues missing)');
    if (typeof cb === 'function') { queueMicrotask(() => cb(err)); return undefined; }
    throw err;
  }
  const out = new Uint8Array(n);
  c.getRandomValues(out);
  const buf = makeBuf(out);
  if (typeof cb === 'function') { queueMicrotask(() => cb(null, buf)); return undefined; }
  return buf;
}
