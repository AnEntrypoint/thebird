export function runAwk(program, stdin, fs_sep) {
  ...
  for (const line of effective) {
    ctx.NR++;
    const trimmed = fs_sep ? line : line.trim();
    const fields = trimmed === '' ? [] : trimmed.split(fs).filter((_, i, a) => i > 0 || a.length === 1 || _ !== '');
...
const AWK_FORBIDDEN_IDENT = /\b(constructor|__proto__|prototype|globalThis|window|self|import|eval|Function)\b/;
...
function matchCond(cond, rec, ctx) {
  ...
  if (AWK_FORBIDDEN_IDENT.test(cond)) return false;
...
function execAction(action, rec, emit, ctx) {
  for (const stmt of action.split(';').map(s => s.trim()).filter(Boolean)) {
    ...
    // $N = value: real awk's most common record-mutation idiom -- assigning a
    // field must rebuild $0 by rejoining $1..NF with OFS (the field separator
    // is NOT preserved on rebuild, matching real awk's documented behavior).
    // This must be checked BEFORE the bare-identifier assign regex below,
    // since '$1' doesn't match that pattern anyway, but keeping it first
    // documents the precedence explicitly.
    const fieldAssign = stmt.match(/^\$(\d+|NF)\s*=\s*(.+)$/);
    if (fieldAssign) {
      const n = fieldAssign[1] === 'NF' ? rec.NF : +fieldAssign[1];
      const val = String(evalExpr(fieldAssign[2], rec, ctx));
      rec.$[n] = val;
      if (n > rec.NF) { for (let i = rec.NF + 1; i < n; i++) if (rec.$[i] == null) rec.$[i] = ''; rec.NF = n; }
      const ofs = ctx.vars.OFS ?? ' ';
      rec.$[0] = rec.$.slice(1, rec.NF + 1).join(ofs);
      continue;
    }
    const nfAssign = stmt.match(/^NF\s*=\s*(.+)$/);
    if (nfAssign) {
      const newNF = Math.max(0, +evalExpr(nfAssign[1], rec, ctx) | 0);
      if (newNF < rec.NF) rec.$.length = newNF + 1;
      else for (let i = rec.NF + 1; i <= newNF; i++) if (rec.$[i] == null) rec.$[i] = '';
      rec.NF = newNF;
      const ofs = ctx.vars.OFS ?? ' ';
      rec.$[0] = rec.$.slice(1, rec.NF + 1).join(ofs);
      continue;
    ...
    if (/\b(for|while|if|else|function)\s*\(|\bfunction\s+[A-Za-z_]/.test(stmt) || /^[A-Za-z_]\w*\s*\[[^\]]*\]\s*=/.test(stmt)) {
      emit('awk: syntax error: unsupported construct \'' + stmt + '\'');
      ctx.error = true;
      continue;
    }
    try { evalExpr(stmt, rec, ctx); } catch { emit('awk: syntax error: unsupported construct \'' + stmt + '\''); ctx.error = true; }
  }
}
...
function evalExpr(expr, rec, ctx) {
  ...
  if (AWK_FORBIDDEN_IDENT.test(s)) return s;
  try {
