export function createReadline({ term, getCompletions, onLine, getPrompt, isBlockOpen, getHistory: injectedGetHistory }) {
  let buf = '';
  let pos = 0;
  let histIdx = -1;
  let escBuf = '';
  ...
  function promptStr() { return '\r\n\x1b[32m' + getPrompt() + PROMPT_SUFFIX + '\x1b[0m'; }
  function showPrompt() { write(promptStr()); }

  function redraw() {
    write('\r\x1b[2K\x1b[32m' + getPrompt() + PROMPT_SUFFIX + '\x1b[0m' + buf);
  ...
  function moveTo(n) {
    ...
    const pLen = getPrompt().length + PROMPT_SUFFIX.length;
  ...
  // createShell always supplies getHistory backed by its own ctx.history array
  // (per-shell-instance state). The window.__debug fallback below only fires
  // for a standalone createReadline caller that didn't inject one.
  function getHistory() { return injectedGetHistory ? injectedGetHistory() : (window.__debug?.shell?.history || []); }
  ...
  function onData(data) {
    ...
    if (pendingHighSurrogate) {
      ...
      const lowCode = data.length === 1 ? data.charCodeAt(0) : -1;
      if (lowCode >= 0xDC00 && lowCode <= 0xDFFF) { insert(high + data); return; }
      // Not a valid low surrogate: flush the lone high surrogate, then fall through to process data normally.
      insert(high);
    }
    if (data.length === 1) {
      const code = data.charCodeAt(0);
      if (code >= 0xD800 && code <= 0xDBFF) { pendingHighSurrogate = data; return; }
    }
